...
## Weapon animation and beam visuals

- Equipped weapon prefabs are animated by the engine, not by mod code. `ElementAnimator`'s constructor (built in `Element.Create`, after `Element.CreateAttachments` mounts `WeaponTemplate.Model` / `ModelSecondary`) runs `GetComponentsInChildren<Animator>()` on the element and wraps each in `AnimatorParameters`, a set of the parameter hashes that Animator declares. Every `ElementAnimator.SetAnimatorBool/Float/Int/Trigger` sets the soldier's Animator, then every attachment Animator declaring the same name. `Skill.PlayAnimation` maps `AnimationType` to the trigger (`ShootSingle` is `Shoot_Single`, `ShootBurst` is `Shoot_Burst`, `ThrowGrenade` is `Throw_Grenade`, `SpecialAttack1..4` are `Special_Attack_N`). Animators mounted later through `AttachPrefab` are not re-collected. Vanilla precedents: `rpg_t1` / `rpg_t2` carry `aco.rpg_t1` (`AmmoCount`, `RefillAmmo`, `Shoot_Single`, a `toggle_hide_rocket` child), every tripod weapon is a SkinnedMeshRenderer rig with an `aco.<weapon>_child` override of `aco.local_forces_medium_mg_parent` (`IsWeaponDeployed`, `CancelDeployment`, `IsWeaponAttached`, `Stance`, `AngleRotation`, `AmmoCount`, `Shoot_Single`, `Shoot_Burst`, `Special_Attack_1`, `EquippedWeapon`). The reverse channel exists too: `HeavyWeaponComp` adds `AnimatorEventRelay` to the Hand_R_Special attachment so weapon clips can raise `Menace.Tactical.AnimationEvent` (`DisableLeftHandIK` 10, `EnableLeftHandIK` 11, `AttachHeavyWeapon` 12, `AttachSkillProjectile` 15, `DetachSkillProjectile` 16).
- `ElementAttachments.DeployWeapon` is a pure reparent (saves the local pose, `SetParent` to the world, restores the world pose) and `PickupDeployedWeapon` reverses it. The tripod unfold is the rig's own `Deployed_In` state reacting to `IsWeaponDeployed`. `HeavyWeaponComp.OnStanceChanged` shows `Back_Special` only while the stance is PinnedDown. The non-tripod rocket launchers (`specialweapon.rocket_launcher_tier1_pal`, the RPGs) are two static meshes, backpack at `Back_Special` and gun at `Hand_R_Special`, `AnimSize Big`, `AnimGrip Vertical`, `IsDeploymentRequired` on the fire skill. `WeaponTemplate.AnimType` (`WeaponAnimType`: MediumMachinegun, RocketLauncher, Autocannon, Mortar, AutoGrenadeLauncher, Minigun, HeavyMachinegun, Laserlance, SelfPropelledGun) is the tripod lever: it switches the soldier to the `_tripod` controller, adds `HeavyWeaponComp` and `passive.encumbered`. There is no shoulder-fired pose enum.
- Every skill visual is a `GameObject` reference on the SkillTemplate: `ProjectileData` (Odin-serialised `BaseProjectileData`, concrete `StandardProjectileData`, `LaserProjectileData`, `ArcProjectileData`, `HomingProjectileData`, `DrunkenProjectileData`, authored with `set "ProjectileData" type="..." { ... }`), `SecondaryProjectileData`, `MuzzleEffect` with `MuzzleType` and `MuzzleEffectOverrides2`, `DefaultImpactEffect`, and `ImpactOnSurface`, a 14-entry list indexed by the engine's surface order (stone, metal, sand, earth, snow, snow, stone, sand, earth, grass, none, vegetation, stone, earth as the vanilla tables read). `LaserProjectileData.CreateInstance` attaches `LaserProjectile` to the prefab and stretches it between muzzle and impact (lifetime 0.5 s, fade in 0.3, fade out 0.75, driven through the material's intensity property), so a beam prefab is a mesh plus an emissive material and nothing else. Red set: `LaserTracerCapsule` (`laser_red_material`), `laser_projectile_tracer_big` (12 units, the construct gunslinger's flying bolt), `laser_muzzle_flash_large_01`, `impact_laser_large_{stone,metal,sand,earth,snow,grass,vegetation}_01`, `laser_sparks`. Blue railgun set: `tracer_railgun`, `muzzle_flash_railgun_blue`, `railgun_ground_dust_earth_01` (`active.shoot_railgun_infantry`, `offmap.shoot_orbital_railgun`). `DecalCollection` fields take no `asset=`, so a clone keeps its parent's decals.
